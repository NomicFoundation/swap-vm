...
contract PeggedSwapMathWrapper {
    ...
    function computeInvariantFromReserves(
        uint256 x,
        uint256 y,
        uint256 x0,
        uint256 y0,
...
contract PeggedSwapTest is Test, OpcodesDebug {
    ...
    function _createOrder(PoolSetup memory setup) internal view returns (ISwapVM.Order memory) {
        ...
        bytes memory programBytes = bytes.concat(
            ...
            setup.feeInBps > 0
                ? prog.build(Fee._flatFeeAmountInXD, FeeArgsBuilder.buildFlatFee(uint32(setup.feeInBps)))
                : bytes(""),
            prog.build(
    ...
    function _assertSwapQuoteConsistency(ISwapVM.Order memory order, uint256 amount, bool isExactIn) internal {
        bytes memory signature = _signOrder(order);
        bytes memory takerData = _makeTakerData(isExactIn, signature);

        // Quote
        (uint256 quotedIn, uint256 quotedOut, ) = swapVM.asView().quote(order, tokenA, tokenB, amount, takerData);

        // Swap
        vm.prank(taker);
        (uint256 swappedIn, uint256 swappedOut, ) = swapVM.swap(order, tokenA, tokenB, amount, takerData);
    ...
    function test_PeggedSwap_EdgeCase_A_Zero() public {
        ...
        uint256 inv1 = PeggedSwapMath.invariantFromReserves(x1, y1, setup.x0, setup.y0, setup.linearWidth);
    ...
    function test_PeggedSwap_SwapAbundantForScarce() public {
        ...
        vm.prank(taker);
        (uint256 amountIn, uint256 amountOut, ) = swapVM.swap(order, tokenA, tokenB, swapAmount, takerData);
    ...
    function test_PeggedSwap_SwapScarceForAbundant() public {
        ...
        vm.prank(taker);
        (uint256 amountIn, uint256 amountOut, ) = swapVM.swap(order, tokenB, tokenA, swapAmount, takerData);
    ...
    function test_PeggedSwap_RoundRobin_DepletionAndRoundingExploit() public {
        ...
        for (uint256 w = 0; w < linearWidths.length; w++) {
            ...
            for (uint256 i = 0; i < drainAmounts.length; i++) {
                vm.prank(taker);
                (uint256 amIn, uint256 amOut, ) = swapVM.swap(order, tokenA, tokenB, drainAmounts[i], takerDataExactIn);
                takerTotalIn += amIn;
                takerTotalOut += amOut;
                balA += amIn;
                balB -= amOut;
            ...
            for (uint256 i = 0; i < edgeAmounts.length; i++) {
                ...
                try swapVM.asView().quote(order, tokenA, tokenB, edgeAmounts[i], takerDataExactIn) returns (
                    uint256,
                    uint256 qOut,
                    bytes32
                ) {
            ...
            for (uint256 i = 0; i < refillAmounts.length; i++) {
                try swapVM.asView().quote(order, tokenB, tokenA, refillAmounts[i], takerDataExactInReverse) returns (
                    uint256,
                    uint256 qOut,
                    bytes32
                ) {
            ...
            for (uint256 round = 0; round < 5; round++) {
                ...
                try swapVM.asView().quote(order, tokenA, tokenB, smallAmtAB, takerDataExactIn) returns (
                    uint256,
                    uint256 qOut,
                    bytes32
                ) {
                ...
                try swapVM.asView().quote(order, tokenB, tokenA, smallAmtBA, takerDataExactInReverse) returns (
                    uint256,
                    uint256 qOut,
                    bytes32
                ) {
