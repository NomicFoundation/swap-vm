...
contract OrderHasher {
    ...
    function orderTypedData(ISwapVM.Order calldata order) public view returns (bytes memory) {
        bytes32 hash = keccak256(
            abi.encode(SwapVM(payable(swapVM)).ORDER_TYPEHASH(), order.maker, order.traits, keccak256(order.data))
        );
    ...
    function _buildDomainSeparator() private view returns (bytes32) {
        return
            keccak256(abi.encode(TYPE_HASH, keccak256(bytes(name)), keccak256(bytes(version)), block.chainid, swapVM));
    }
