...
contract VeryImbalancedDifferentDecimals is PeggedFeesInvariants {
    ...
    function test_AsymmetricPool_ReverseSwap_NoAxisMismatch() public {
        ...
        try swapVM.asView().quote(order, tokenInForward, tokenOutForward, swapAmount, exactInData) returns (
            uint256,
            uint256 outForward,
            bytes32
        ) {
            ...
            assertLe(
                ...
                string.concat(
                    ...
                    "Output: ",
                    vm.toString(outForwardScaled),
                    ", Max reasonable: ",
                    vm.toString(maxReasonableOutput)
                )
            );
        } catch {
        ...
        try swapVM.asView().quote(order, tokenOutForward, tokenInForward, swapAmount, exactInData) returns (
            uint256,
            uint256 outReverse,
            bytes32
        ) {
            ...
            assertLe(outReverseScaled, swapAmount * 20, "Reverse direction also should not have axis mismatch");
        } catch {
