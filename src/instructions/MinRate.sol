...
library MinRateArgsBuilder {
    ...
    function parse(
        ...
        address tokenIn,
        address tokenOut
    ) internal pure returns (uint64 rateIn, uint64 rateOut) {
...
contract MinRate {
    ...
    function _requireMinRate1D(Context memory ctx, bytes calldata args) internal {
        require(
            ...
            MinRateExpectedBeforeSwapAmountsComputed(ctx.swap.amountIn, ctx.swap.amountOut)
        );
    ...
    function _adjustMinRate1D(Context memory ctx, bytes calldata args) internal {
        ...
        require(
            ctx.swap.amountIn > 0 && ctx.swap.amountOut > 0,
            MinRateRunLoopExpectToComputeSwapAmounts(ctx.swap.amountIn, ctx.swap.amountOut)
        );
